...
#import "KBMAuthenticationDelegate.h"
 ...
 */
extern NSString *const KBMMenuItemCamera;
 ...
 *
 *  @param integrationId A valid integration id retrieved from the BrandMessenger web portal.
 *  @param authCode A valid auth code generated from the BrandMessenger API.
 */
 ...
 *
 *  @discussion App id can be retrieved from the BrandMessenger web portal
 *
 ...
 *
 *  @discussion integration id can be retrieved from the BrandMessenger web portal
 *
 *  This value may only be set once, and must be set at init time.
 */
 ...
 *
 *  @discussion Auth code can be retrieved from the BrandMessenger API
 *
 *  This value may only be set once, and must be set at init time.
 */
...
/**
 *  @abstract The BrandMessenger region for this account.
 *
 ...
 *
 *  @discussion Valid values are KBMMenuItemCamera, KBMMenuItemGallery, KBMMenuItemDocument and KBMMenuItemLocation
 *
 ...
 *
 *  @discussion Setting this to `YES` allows the user to send messages even when the device is offline. It also prevents an network error banner from showing when offline. This setting only applies to the conversation view. Will automatically resend failed message when network is restored.
 *
 */
@property BOOL allowOfflineUsage;

/**
 *  @abstract Keeps app running in background to sync when network is restored
 *
 *  @discussion Set this to 'YES' with allowOfflineUsage. This will enable geolocation tracking to keep the application in the background, able to then detect network reachability changes.
 *
 */
@property BOOL allowOfflineUsageInBackground;
 ...
 *
 *  @discussion When set to YES, BrandMessenger will swizzle the following methods of your UIApplicationDelegate in order to automatically handle push notification receiving and registering, as well as user notification handling.
 *
 ...
 *
 *  1. When a new push token is received in -application:didRegisterForRemoteNotificationsWithDeviceToken:, you must call BrandMessenger +setPushToken.
 *  2. In your -application:didReceiveRemoteNotification: or -application:didReceiveRemoteNotification:fetchCompletionHandler: callback, you must call BrandMessenger +handlePushNotification: with the passed userInfo dictionary.
 *  3. In your -application:handleActionWithIdentifier:forRemoteNotification:withResponseInfo:completionHandler: callback, you must call BrandMessenger +handleUserNotificationActionWithIdentifier:withResponseInfo:completionHandler: with the correct parameters.
 *
 ...
 *
 *  @discussion For more information, see the +userNotificationCenterDelegate method of the BrandMessenger class.
 *
 ...
 *  This value may only be set once, and must be set at init time.
 *
 *  @see KBMAuthenticationDelegate
 */
@property(weak, nonatomic, nullable) id<KBMAuthenticationDelegate> authenticationDelegate;

/**
 *  @abstract Set to use unformatted deviceToken without stripping special chars
 *
 *  @discussion To match older unformatted deviceToken string when deviceToken.description used to work.
 *
 *  The default value should be NO.
 */
@property BOOL useUnformattedAPNDeviceToken;
