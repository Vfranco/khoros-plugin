 ...
 *
 *  @discussion Unlike `externalId`, this property is set automatically by BrandMessenger, and is not configurable. This is analogous to `appUser._id` in the REST API response. A user is assigned an `userId` once they start a conversation or when an existing user is logged in by calling +login:jwt:completionHandler:.
 *
 *  @see BrandMessenger
 */
